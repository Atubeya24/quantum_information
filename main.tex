...
\\
以上を通じて、本論文では量子情報理論を「何が量子的資源であり」、「なぜそれが有用であり」、「どのように実現されるのか」という三つの観点から統一的に理解することを目指す。本論は基本的には\cite{Nielsen:2012yss}の流れに基づくが、議論が不足であると感じた、及び最新の内容に触れる場合は、適宜それを別個の論文で保管する形で議論を進めていく。\\
\\
...
\section{量子テレポーテーション}
量子もつれを利用した現象として、代表的なものに量子テレポーテーションが存在する。例えば、詳細を知らない量子ビット$\ket{\phi}=\alpha \ket{0}+\beta \ket{1}$を、他者に会わずに送信したいと考える。量子テレポーテーションとは、もつれを活かしてこの送信の手続きを行う方法である。\\
\\
元々、量子通信理論とは、
\begin{enumerate}
    \item 如何に多くの量子情報を
    ...
    \item 如何にノイズを掻い潜り
\end{enumerate}
量子テレポーテーションを通じて交換するかを追求する学問であった。故に、量子通信理論のひとまずのゴールとして、一番基礎的な、この量子テレポーテーションについての考察を行う。大抵の場合、テレポーテーションはBellを用いた簡単なセットアップを俯瞰することしかしないが、ここでは通信理論らしく、一般の状態を通信源とした、テレポーテーションの性能評価まで行う。

\subsection{Bell-stateによる量子テレポーテーション}
量子テレポーテーションの有名なセットアップとして、Bell-stateを利用したセットアップが知られている。\\
\begin{setupbox}{Bell-stateによる量子テレポーテーション}
\begin{enumerate}
    \item 初期状態を用意\\
    送信したい量子ビットを系A',Bell状態にある状態を複合系ABであるとする。この時、複合系A'Aを局所的であるとし、系BはA'Aから空間的に離れているとする。この時、全体系AA'Bの状態$\ket{\psi}$は以下のように書ける。
\begin{equation}\begin{split}
    \ket{\psi}_{AA'B} =\ket{\phi}_{A'}\ket{Bell}_{AB} 
    =\frac{1}{\sqrt{2}} (\alpha\ket{0}_{A'}+\beta \ket{1}_{A'})(\ket{00}_{AB}+\ket{11}_{AB})
\end{split} \end{equation}
    \item A側の操作\\
    制御ビットをA'、標的ビットをAをしてCNOTゲートを作用させ、及び系A'に対してアダマールゲートを作用させる。
\begin{equation}\begin{split}
    \ket{\psi'}&=H_{A}C_{A}^{A'}\ket{\psi}_{AA'B}\\&=\frac{1}{2}(\ket{00}_{A'A}(\alpha \ket{0}_{B}+\beta \ket{1}_{B})
    \\&+\ket{01}_{A'A}(\beta \ket{0}_{B}+\alpha \ket{1}_{B})\\
    &+\ket{10}_{A'A}(\alpha \ket{0}_{B}-\beta \ket{1}_{B})\\
    &+\ket{11}_{A'A}(-\beta \ket{0}_{B}+\alpha \ket{1}_{B}))
\end{split} \end{equation}
    \item AA'側の観測、B側の操作\\
    AA'側で、「00」が観測された際は、Bに何もしない、「01」が観測された際は、BにXゲートを作用させ、「10」が観測された際はBにZゲートを作用させ、「11」が観測された際はBにXゲート,Zゲートを順番に作用させる。
\end{enumerate}
終状態：$\ket{f}_{B}=\ket{\phi}=\alpha\ket{0}+\beta\ket{1}$
\end{setupbox}
\begin{proofbox}
   系AA'で観測された状態と、各状態が観測された際のBの状態を整理する。
\begin{equation}\begin{split}
    00 &\to \alpha \ket{0}_{B}+\beta \ket{1}_{B}=\ket{\phi}\\
    01 &\to \beta \ket{0}_{B}+\alpha \ket{1}_{B}=X\ket{\phi}\\
    10 &\to \alpha \ket{0}_{B}-\beta \ket{1}_{B}=Z\ket{\phi}\\
    11 &\to -\beta \ket{0}_{B}+\alpha \ket{1}_{B}=XZ\ket{\phi}
\end{split} \end{equation}
この時、「00」が観測された際は何もしない、「01」が観測された際はXゲートを作用させ、「10」が観測された際はZゲートを作用させ、「11」が観測された際はXゲート,Zゲートを順番に作用させるとする。すると、この手続きにより、部分系Bの状態は、どの観測結果においても$\ket{\phi}$となることが理解できる。
\end{proofbox}

量子テレポーテーションは極めて多くの興味ある哲学的な性質を含んでいる。\\
\\
第1に、これは観測する瞬間に空間的に離れたもう一方の系の状態が定まり、情報が伝わるという、一見すると因果律を破っているように見える点である。\\
しかし、これは部分系B視点では、古典通信がくるまで、何も情報が伝わっていないことが、重要となる。このため、情報を伝える速度が光速を超えることは決してない。\\
\\
...
\end{theorembox}
\begin{proofbox}
仮に、任意のAの量子状態$\ket{\psi_{A}}$をBに複製できるクローン操作$U_{copy}$があったとする。\\
\\
$U_{copy}$は任意の状態をコピーできるので、用意した2つの量子状態$\ket{\psi},\ket{\phi}$のコピーも可能である。この時、それぞれの量子状態の変化は以下のように書ける。
 \begin{equation}\begin{split}
     \ket{\psi}_{A}\ket{s}_{B}\to&U_{copy}(\ket{\psi}_{A}\ket{s}_{B}) = \ket{\psi}_{A}\ket{\psi}_{B}\\
     \ket{\phi}_{A}\ket{s}_{B}\to&U_{copy}(\ket{\phi}_{A}\ket{s}_{B}) = \ket{\phi}_{A}\ket{\phi}_{B}\\
 \end{split} \end{equation}
クローン操作はユニタリーゲートでなければならないため、内積が保存されなければならない。この時、2つの量子状態の内積は、コピーの前後で不変であるため、
\begin{equation}\begin{split}
    \bra{\psi}\ket{\phi}\bra{s}\ket{s} = (\bra{\psi}\ket{\phi})^{2}\implies \bra{\psi}\ket{\phi}=0,1
\end{split} \end{equation}
従って、任意の状態を複製できるというデバイスが存在するという仮定は誤りであり、仮に何かしらの状態を複製できるデバイスが存在するなら、その状態もしくはそれと直交する状態のみを複製することが出来る\footnote{ただし、特定の状態をコピーする回路が禁止されていない点は重要である。もつれの定量化で確認した通り、量子情報において状態のコピーを用いて議論することがしばしばある。}。
\end{proofbox}
量子テレポーテーションは一見この非クローン定理に反しているように見えるが、実はそうではない。部分系AA'の状態が観測により定まりきっており、量子のもつ位相の情報が全てBに伝わっている。

\subsection{Bell-state Fidelity とテレポーテーション性能}
量子もつれの定量化の概念を知った今なら、例えば「一般の共有状態をteleportation resourceとして用いたとき、その性能はどのように特徴づけられるだろうか」といった疑問が生じるはずである。\\
\\
これに対する答えとして、「最大エンタングル状態とのFidelityが、teleportationの最大平均Fidelityを直接決定する」というstatementが知られている\cite{}。この節では、この厳密な関係を与える。\\
しかし、これを行うためには、考えうる全てのテレポーテーションのチャンネルを考えねばならない。これには一般に大変な労力を伴うが、いくつかの道具を整理すると、今回の場合はかなり単純な問題に帰着できる。

\subsubsection{state-channel correspondence}
本節では、channel操作をまずは状態として操作する方法論を提示する。これを用いて、しばしチャンネルの言葉をstateの言葉に直して議論することが出来る。
\begin{defbox}{State--channel correspondence}
$d$ 次元系上の量子チャンネル $\Lambda$ に対して、
\begin{align*}
    \rho_\Lambda
    \equiv
    (I\otimes\Lambda)(P_+),
    \qquad
    P_+=\ket{\Phi_+}\bra{\Phi_+},
\end{align*}
と定義する。ここで
\begin{align*}
    \ket{\Phi_+}
    =
    \frac{1}{\sqrt d}\sum_{i=1}^{d}\ket{ii}
\end{align*}
は最大エンタングル状態である。
\end{defbox}
\begin{resultbox}{State-channel correspondenceの一意性}
量子チャンネル $\Lambda$ は、二体系状態 $\rho_\Lambda$ として表すことができる。
逆に、$\rho_\Lambda$ から元のチャンネルは
\begin{align*}
    \boxed{
    \Lambda(\sigma)
    =
    d\,\Tr_A
    \left[
        (\sigma^T\otimes I)\rho_\Lambda
    \right]
    }
\end{align*}
と復元できる。したがって、この対応は一対一であり、
\begin{align*}
    \boxed{
    \Lambda_1=\Lambda_2
    \iff
...
\begin{align*}
    ...
    &=
    \sum_{i,j}
    \sigma_{ij}\Lambda(\ket{i}\bra{j})\\
    &=
    \Lambda(\sigma).
\end{align*}
よって $\rho_\Lambda$ から $\Lambda$ は一意に復元できる。
\end{proofbox}

\subsubsection{チャンネル操作論}
次に、量子テレポーテーションのfidelityを議論するために、必要なチャンネルを定義し、その性質を述べる。\\
\\
まず、量子チャンネル $\Lambda$ の性能を特徴づける二つの Fidelity を導入する。$f(\Lambda)$ は任意の純粋状態に対する平均 Fidelity、$F(\Lambda)$ は最大エンタングル状態に対する Fidelity である。
\begin{defbox}{チャンネルの Fidelity}
\begin{align*}
f(\Lambda)
&\equiv
\int d\phi,
\bra{\phi}
\Lambda(\ket{\phi}\bra{\phi})
\ket{\phi},\
F(\Lambda)
&\equiv
\bra{\Phi_+}
...
次に、後に一般のチャンネルを帰着させる脱分極チャンネルを定義する。
\begin{defbox}{脱分極チャンネル}
\begin{align*}
\Lambda_p^{\mathrm{dep}}(\rho)
=p\rho+(1-p)\frac{I}{d}.
\end{align*}
\end{defbox}
特に、脱分極チャンネルに対応するstateは、以下のように記述できる。
また、脱分極チャンネルに対応する状態は
\begin{align*}
\rho_{\Lambda^{dep}_{p}}
&\equiv
(I\otimes\Lambda_p^{\mathrm{dep}})
(\ket{\Phi_+}\bra{\Phi_+})
=
p\ket{\Phi_+}\bra{\Phi_+}
+(1-p)\frac{I\otimes I}{d^2}
\end{align*}
であり、noise を含む最大エンタングル状態として解釈できる。\\
\\
最後に、一般のチャンネルを脱分極チャンネルへ帰着させる操作として、channel twirling を導入する。
\begin{defbox}{Channel twirling}
\begin{align*}
\operatorname{twirl}(\Lambda)(\rho)
\equiv
\int dU
U^\dagger
\Lambda(U\rho U^\dagger)
U 
\end{align*}
\end{defbox}
これらの道具立てから、以下の二つの重要な補題が成立する。
\begin{theorembox}{Twirlingと脱分極チャンネルの関係}
任意のチャンネル $\Lambda$ に対して、
\begin{align*}
    \operatorname{twirl}(\Lambda)
    =
    \Lambda_{p}^{\mathrm{dep}}
\end{align*}
となる $p$ が存在する。
\end{theorembox}

\begin{proofbox}
証明は、state--channel correspondence を用いて以下の三段階で行う。

\begin{setupbox}{証明のセットアップ}
以下の三つを示せば十分である。
\begin{enumerate}
    \item $\operatorname{twirl}(\rho)=\rho_F$
    \item $\Lambda_{\rho_F}=\Lambda_p^{\mathrm{dep}}$
    \item $\rho_{\operatorname{twirl}(\Lambda)}
    =\operatorname{twirl}(\rho_\Lambda)$
\end{enumerate}
ここで
\begin{align*}
    F&=\Tr(\rho P_+),\\
...
したがって、これらが成立すれば
\begin{align*}
    \rho_{\operatorname{twirl}(\Lambda)}
    &=
    \operatorname{twirl}(\rho_\Lambda)\\
    &=
    \rho_F\\
    &=
    \rho_{\Lambda_p^{\mathrm{dep}}},
\end{align*}
となるため、state--channel correspondence の一意性から
\begin{align*}
    \operatorname{twirl}(\Lambda)
    =
    \Lambda_p^{\mathrm{dep}}
...
\begin{enumerate}
\item $\operatorname{twirl}(\rho)=\rho_F$ の証明
...
\begin{align*}
    ...
    &=
    \frac{1}{\sqrt{d}}
    \sum_{j,k}(UU^\dagger)_{jk}\ket{jk}\\
    &=
    \ket{\Phi_+}
\end{align*}
が成立する。
...
\begin{align*}
    a
    &=
    \Tr[P_+\operatorname{twirl}(\rho)]\\
    &=
    \Tr(P_+\rho)
    \equiv F.
\end{align*}
さらに $\Tr[\operatorname{twirl}(\rho)]=1$ より、
\begin{align*}
    b=\frac{1-F}{d^2-1}.
\end{align*}
よって
...
\item $\Lambda_{\rho_F}=\Lambda_p^{\mathrm{dep}}$ の証明

state--channel correspondence より、
\begin{align*}
    \Lambda_{\rho_F}(\sigma)
    =
    d\,\Tr_A[(\sigma^T\otimes I)\rho_F].
\end{align*}
ここで
\begin{align*}
    d\,\Tr_A[(\sigma^T\otimes I)P_+]
    &=
    \sigma,\\
    \Tr_A(\sigma^T\otimes I)
    &=
    \Tr(\sigma)I
\end{align*}
を用いると、
\begin{align*}
    \Lambda_{\rho_F}(\sigma)
    &=
    \frac{d^2F-1}{d^2-1}\sigma
    +
    \frac{d(1-F)}{d^2-1}\Tr(\sigma)I.
\end{align*}
$\Tr\sigma=1$ に対して、
\begin{align*}
    \Lambda_{\rho_F}(\sigma)
    &=
    p\sigma+(1-p)\frac{I}{d}\\
    &=
    \Lambda_p^{\mathrm{dep}}(\sigma),
\end{align*}
ただし
\begin{align*}
    \boxed{
...
\item
$\rho_{\operatorname{twirl}(\Lambda)}
=\operatorname{twirl}(\rho_\Lambda)$ の証明

$\rho_\Lambda=(I\otimes\Lambda)(P_+)$ と
$(U\otimes U^*)\ket{\Phi_+}=\ket{\Phi_+}$ を用いると、
\begin{align*}
    \rho_{\operatorname{twirl}(\Lambda)}
    &=
    (I\otimes\operatorname{twirl}(\Lambda))(P_+)\\
    &=
    \int dU\,
    (U\otimes U^*)\rho_\Lambda
    (U^\dagger\otimes U^T)\\
    &=
    \operatorname{twirl}(\rho_\Lambda).
\end{align*}
...
\begin{theorembox}{Twirlingによる平均Fidelityの不変性}
任意のチャンネル $\Lambda$ に対して、
\begin{align*}
    \boxed{
    f(\Lambda)
    =
    f(\operatorname{twirl}(\Lambda))
    }
\end{align*}
が成立する。
\end{theorembox}
...
\begin{align*}
    ...
    \ket{\phi}.
\end{align*}
ここで $\ket{\psi}=U\ket{\phi}$ とおく。
Haar 測度のユニタリ不変性より $d\psi=d\phi$ であるため、
\begin{align*}
    f(\operatorname{twirl}(\Lambda))
    &=
    \int d\psi\,
...
以上の二つの補題を用いることで、
任意のチャンネルに対して平均 Fidelity と
Bell-state に対する Fidelity の関係が得られる。

\begin{theorembox}{任意のチャンネルのFidelity}
任意の $d$ 次元量子チャンネル $\Lambda$ に対して、
\begin{align*}
    \boxed{
    f(\Lambda)
    =
    \frac{dF(\Lambda)+1}{d+1}
    }
\end{align*}
が成立する。
\end{theorembox}
...
\begin{proofbox}
Twirling と脱分極チャンネルの関係、および
Twirling による平均 Fidelity の不変性から、
\begin{align*}
    f(\Lambda)
    &=
    f(\operatorname{twirl}(\Lambda))\\
    &=
    f(\Lambda_p^{\mathrm{dep}}).
\end{align*}
脱分極チャンネルについて、
\begin{align*}
    f(\Lambda_p^{\mathrm{dep}})
    ...
    p+\frac{1-p}{d^2}.
\end{align*}
また、Twirling は Bell-state に対する Fidelity も保存するため、
\begin{align*}
    F(\Lambda_p^{\mathrm{dep}})
    =
    F(\Lambda).
...
\subsubsection{Fidelityの証明}
以上の道具立てを用いて、
共有状態 $\rho$ を用いたテレポーテーションで達成可能な
最大平均 Fidelity を求める。

まず、$\rho$ に trace-preserving LOCC $\mathcal{L}$ を作用させて
得られる最大 Bell-state Fidelity を
\begin{align*}
    F_{\max}(\rho)
    \equiv
    \max_{\mathcal{L}}
...
また、$\rho$ を資源として用いる任意のテレポーテーション
プロトコルによって実現されるチャンネルを $\Lambda$ とし、
その最大平均 Fidelity を
\begin{align*}
    f_{\max}(\rho)
    \equiv
...
\begin{theorembox}{テレポーテーションにおける最大 Fidelity}
$d$ 次元系の共有状態 $\rho$ に対して、
テレポーテーションで達成可能な最大平均 Fidelity は
\begin{align*}
    \boxed{
    f_{\max}(\rho)
    =
    \frac{dF_{\max}(\rho)+1}{d+1}
    }
\end{align*}
で与えられる。
\end{theorembox}
...
$\rho$ を用いた任意のテレポーテーションプロトコルが
チャンネル $\Lambda$ を実現するとする。
最大エンタングル状態 $P_+$ の片側にこのチャンネルを作用させると、
\begin{align*}
    \rho_{\Lambda}
    =
    (I\otimes\Lambda)(P_+)
...
この操作全体は $\rho$ に対する trace-preserving LOCC として
実現できるため、$F_{\max}$ の定義から
\begin{align*}
    F(\Lambda)
    =
    F(\rho_{\Lambda})
...
次に、この上界が達成可能であることを示す。

まず $\rho$ に最適な trace-preserving LOCC を作用させ、
\begin{align*}
    F(\rho')
    =
    F_{\max}(\rho)
...
この状態は脱分極チャンネル
$\Lambda_p^{\mathrm{dep}}$ に対応するため、
$\rho_{F_{\max}}$ を用いた標準テレポーテーションは
$\Lambda_p^{\mathrm{dep}}$ を実現する。

したがって、その平均 Fidelity は
\begin{align*}
    f
    &=
    \frac{dF(\rho_{F_{\max}})+1}{d+1}\\
    &=
    \frac{dF_{\max}(\rho)+1}{d+1}.
\end{align*}
...
を得る。
\end{proofbox}
最後に、$f_{max}=1$となる条件を確認する。\\
\\
上式から、これは$F_{max}=1$でなければならない。LOCCの下ではエンタングルは減る一方であるため、$F_{max}=1$を達成するためには、共有状態は最大エンタングルでなければならない。\\
\\
故に、完全なテレポーテーションを達成するためには、最大エンタングル状態を導入する必要がある。このため、例えば1状態のテレポーテーションを行うには、Bell状態が必要不可欠なのである。\\
\\
最後に一つコメントを行う。前節で量子もつれの定量化としてEEを定義したにもかかわらず、ここでEEではなく性能評価の項にfidelityが出てくるのは何故か、である。答えとしては、リソースが仮に
\begin{enumerate}
    \item 純粋状態の場合\\
    この場合、Fidelityとは$\frac{1}{2}$-renyi entropyそのものなので、実はEEとほぼ同じ意味である。
    \item 混合状態の場合\\
    この場合、EEはもつれの定量化としての意味を持たないが、Fidelityはその意味を持つ。
\end{enumerate}
このため、もつれの定量化の尺度として、EEをより包括したものがFidelityであるためである。
...
\subsection{断熱準備}
前節で時間発展を量子回路上で表したが、この時間発展回路をどのように生かすべきであろうか？本節では、この時間発展を活かした最たるアルゴリズムとして、「断熱準備」\cite{Farhi:2000ikn}を導入し、実際に物理量を計算する。\\
\\
...
\begin{figure}[h]
    ...
    \label{ising_zika}
\end{figure}
厳密解は以下のように与えられる。ただし、これは文献\cite{Pfeuty:1970}から引っ張ってきた。
\begin{equation}\begin{split}
...
\begin{figure}[h]
    \centering    \includegraphics[height=5cm]{J_dependence_of_energy.eps}
    \caption{エネルギーのJ依存性}
    \label{ising_energy}
\end{figure}
...
先ほど求めた横地場イジングモデルの基底状態に対して2次renyiエントロピーのアルゴリズムを適用する。まず粒子数N、スピン鎖を分割した際の分割空間内の粒子数Lとした際、$N,L \to \infty$の場合の２次renyiエントロピーの厳密解は以下の通り\cite{Franchini:2007eu}。
\begin{equation}\begin{split}
...
そこで本章では、多くの量子アルゴリズムを包括的に理解できる枠組みである、量子特異値変換（Quantum Singular Value Transformation; QSVT）を、主に\cite{Low:2016sck}\cite{Low:2016znh}\cite{Gilyen:2018khw}に基づき解説する。QSVTを理解することで、既存の量子アルゴリズムを統一的な観点から捉えられるだけでなく、新たな量子アルゴリズムを設計するための基本的な指針を得ることができる。\\
\\
...
このBlock-encodingを用いた手法を考えることで、ユニタリであるという制約に縛られることなく、より広いクラスの関数を実装できる。最初にこの方法が提示された論文は\cite{Low:2016sck}である。\\
\\
...
\end{enumerate}
少なくとも、Block-encodingからダイレクトで与えられたUは、上記の2乗の計算から「避けなければならない問題」を含んでいる。この問題を回避するために、Block-encoding回路Uを「改造」しなければならない。この改造した回路のことを、原論文\cite{Low:2016znh}に習い「iterate W」と呼ぶことにする。以下では、iterateの上手い構成方法、「Qubitization」を与える。
...
\begin{defbox}{回転iterate}
    \begin{align*}
        W_{\phi} &\equiv Z_{\phi-\frac{\pi}{2}} WZ_{-\phi+\frac{\pi}{2}} \\&= \bigoplus_{\lambda} \begin{pmatrix}
            \lambda & -ie^{-i\phi}\sqrt{1-\lambda^{2}}\\
            -ie^{i\phi}\sqrt{1-\lambda^{2}} & \lambda
        \end{pmatrix} 
        \\&= \bigoplus_{\lambda} \begin{pmatrix}
            cos\theta_{\lambda} & -ie^{-i\phi}sin\theta_{\lambda}\\
            -ie^{i\phi}\sin\theta_{\lambda} & cos\theta_{\lambda}
        \end{pmatrix} 
    \end{align*}
    ただし、ZはHilbert空間$
...
各々のアルゴリズムを見る前に、上記の手続きを行った際に、$\phi$を変えることによって、どのような関数を実装できるか、及び求めたい関数から逆算して$\phi$の値を探り当てる方法はあるか？といった一般論を与えておくことにする。ここでの議論は主に\cite{Low:2016znh}を参考にしている。\\
\\
第一歩として、iterate Wを複数かけた際に、行列がどのような形となりうるかを議論する。このために、L次QSP関数を、以下のようにべき分解する。
\begin{align*}
    W_{\vec{\phi}}[\theta_{\lambda}]&=W_{\phi_{L}}W_{\phi_{L-1}}\cdots W_{\phi_{1}}\\
    &=\sum_{j=0}^{L}(-i)^{j}cos^{L-j}\theta_{\lambda}sin^{j}\theta_{\lambda}\hat{\Phi}_{L,j}
\end{align*}
この時、展開係数$\hat{\Phi}_{L,j}$は以下のような性質を満たす。
\begin{resultbox}{QSP係数の性質とQSP漸化式}
QSP関数は以下のような形に分解可能である。
\begin{align*}
    \hat{\Phi}_{L,j}=(\sigma_{x})^{j}(Re(\Phi_{L,j})I+iIm(\Phi_{L,j})\sigma_{z})
\end{align*}
また、係数$\Phi_{L,j}$は、$\phi_{k}$を与えられた際に以下のように漸化式的に定まる。この漸化式を以降、「QSP漸化式」と呼ぶことにする。
\begin{align*}
    \Phi_{k,j}=\Phi_{k-1,j}+\Phi_{k-1,j-1}e^{i(-1)^{j+1}\phi_{k}}
\end{align*}
ただし、$\Phi_{0,0}=1,\Phi_{i,j>i}=0,\Phi_{i,-1}=0$であるとする。
\end{resultbox}
\begin{proofbox}
    帰納的に示す。
    \begin{enumerate}
        \item L=0の時
        \begin{align*}
            W_{\vec{\phi}}=I\iff \Phi_{0,0}=1
        \end{align*}
        故に満たす。
        \item L=k-1まで満たすとき\\
        \begin{align*}
            W_{\phi_{k-1}}W_{\phi_{k-2}}\cdots W_{\phi_{1}}=\sum_{j=0}^{k-1}(-i)^{j}cos^{k-1-j}\theta_{\lambda}sin^{j}\theta_{\lambda}\hat{\Phi}_{k-1,j}
        \end{align*}
        この時、以下を考える。まずは定義より、
        \begin{align*}
            W_{\phi_{k}}W_{\phi_{k-1}}\cdots W_{\phi_{1}}=\sum_{j=0}^{k}(-i)^{j}cos^{L-j}\theta_{\lambda}sin^{j}\theta_{\lambda}\hat{\Phi}_{k,j}
        \end{align*}
        次に、展開した結果より、
        \begin{align*}
            &W_{\phi_{k}}W_{\phi_{k-1}}\cdots W_{\phi_{1}}\\
            =&(\sum_{j=0}^{k-1}(-i)^{j}cos^{k-1-j}\theta_{\lambda}sin^{j}\theta_{\lambda}\hat{\Phi}_{k-1,j})(cos\theta_{\lambda}-isin\theta_{\lambda}(cos\phi_{L}\hat{\sigma}_{x}+sin\phi_{L}\hat{\sigma}_{y}))\\
            =&(\sum_{j=0}^{k-1}(-i)^{j}cos^{k-j}\theta_{\lambda}sin^{j}\theta_{\lambda}\hat{\Phi}_{k-1,j})\\
            &+(\sum_{i=1}^{k}(-i)^{j}cos^{k-i}\theta_{\lambda}sin^{i}\theta_{\lambda}(cos\phi_{L}\hat{\sigma}_{x}+sin\phi_{L}\hat{\sigma}_{y})\hat{\Phi}_{k-1,i-1})
        \end{align*}
        ただし、最後の2つ目の総和において、$i=j+1$とした。初期条件などから、$\Phi_{k-1,k}=\Phi_{k-1,-1}=0$であるため、総和の添字を変更することができ、
        \begin{align*}
            W_{\phi_{k}}W_{\phi_{k-1}}\cdots W_{\phi_{1}}
            =\sum_{j=0}^{k}(-i)^{j}cos^{k-i}\theta_{\lambda}sin^{i}\theta_{\lambda}\big(\hat{\Phi}_{k-1,j}+(cos\phi_{L}\hat{\sigma}_{x}+sin\phi_{L}\hat{\sigma}_{y})\hat{\Phi}_{k-1,i-1} \big)
        \end{align*}
        両辺同じ係数部分を比較して、
        \begin{align*}
            \hat{\Phi}_{k,j}=\hat{\Phi}_{k-1,j}+(cos\phi_{L}\hat{\sigma}_{x}+sin\phi_{L}\hat{\sigma}_{y})\hat{\Phi}_{k-1,i-1} 
        \end{align*}
        あとは、上式に$\hat{\Phi}_{i,j}$の式を代入して比較すると、
        \begin{align*}
            Re(\Phi_{k,i})&=Re(\Phi_{k-1,i}+cos\theta\Phi_{L-a,j-1})\\
            Im(\Phi_{k,i})&=Im(\Phi_{k-1,i}+(-1)^{j+1}sin\phi_{L}\Phi_{k-1,j-1})\\
            \\
            \implies \Phi_{k,i}&=\Phi_{k-1,j}+\Phi_{k-1,j-1}e^{i(-1)^{j+1}\phi_{k}}
        \end{align*}
    \end{enumerate}
\end{proofbox}
以後、簡単のために以下のようなnotationを採用する。
\begin{align*}
    x&=cos\theta_{\lambda}\\
    y&=sin\theta_{\lambda}
\end{align*}
この時、Wは以下のようにまとまることが理解できる。
\begin{resultbox}{QSPの関数系}
QSPはL次多項式(A,B,C,D)を用いて、以下のように表せられる。
    \begin{align*}
        W_{\vec{\phi}}=
        \begin{cases}
            A(x)I+B(x)\sigma_{z}+C(y)\sigma_{x}+D(y)\sigma_{y}\quad(L\in odd)\\
            A(x)I+B(x)\sigma_{z}+xC(y)\sigma_{x}+xD(y)\sigma_{y}\quad (L\in even)
        \end{cases}
    \end{align*}
\end{resultbox}
\begin{proofbox}
    Lを偶奇に分けた際、jをさらに偶奇に分ける。$\hat{\Phi}_{L,j}$の具体系から、jの偶奇で$\hat{\Phi}_{L,j}$がどの部分の線形結合となるかが変化することがわかる。
    \begin{align*}
        \hat{\Phi}_{L,2n}&=A+B\sigma_{z}\\
        \hat{\Phi}_{L,2n+1}&=C\sigma_{x}+D\sigma_{y}
    \end{align*}
    そして、変数部分については、sin,cosが2乗の形で出現した際、$cos^{2}+sin^{2}=1$を介してcosをsinに押し付ける、といった動作を行う。Lが奇数の時は、L-j,jのどちらかが偶数になるので、その性質を用いて、cosかsinのどちらかに押し付けることが可能である。Lが偶数の時は、あらかじめcosを括り出してやると、同じ処方を与えることができる。\\
    \\
    以上をまとめると、確かにWを上記の形にまとめることができる。
\end{proofbox}
これは、まさにQSPで出現した関数系そのものである。\\
\\
さて、ではQSPの実装条件の本題に入るとする。命題は、以下の通りである。
\begin{problembox}{QSP実装条件}
    関数WをQSPを用いて実装したいと思った時、その関数を実装できるためのWの必要十分条件は？また、Wから実装の際に$\vec{\phi}$を逆算する方法は？
\end{problembox}
以下では、まず、Wが
\begin{align*}
        W_{\vec{\phi}}=
        \begin{cases}
            A(x)I+iB(x)\sigma_{z}+iC(y)\sigma_{x}+iD(y)\sigma_{y}\quad(L\in odd)\\
            A(x)I+iB(x)\sigma_{z}+ixC(y)\sigma_{x}+ixD(y)\sigma_{y}\quad (L\in even)
        \end{cases}
\end{align*}
の形で与えられる、つまり(A,B,C,D)全てが求まっている際、実装可能性の必要十分条件を与える。
\begin{resultbox}{QSP実装可能条件}
    (A,B,C,D)が与えられた際、実装可能であることの必要十分条件は、以下の通りである。
    \begin{enumerate}
        \item (A,B,C,D)が実関数である。
        \item $A(1)=1,B(1)=0$を満たす。
        ...
        \item\begin{enumerate}
            \item Lが奇数のとき
            \begin{align*}
                A(x)^{2}+B(x)^{2}+C(y)^{2}+D(y)^{2}=1
            \end{align*}
            \item Lが偶数の時
            \begin{align*}
                A(x)^{2}+B(x)^{2}+x^{2}C(y)^{2}+x^{2}D(y)^{2}=1
            \end{align*}
        \end{enumerate}
    \end{enumerate}
\end{resultbox}
\begin{proofbox}
    十分条件は$\phi_{L}$を逆算するアルゴリズムの提示そのものであるため、別個に示す。ここでは、必要条件のみ求める。
    \begin{enumerate}
        \item 1,3について\\
        $\hat{\Phi}_{L,j}$の具体系を、QSP関数
        \begin{align*}
    W_{\vec{\phi}}[\theta_{\lambda}]&=W_{\phi_{L}}W_{\phi_{L-1}}\cdots W_{\phi_{1}}\\
    &=\sum_{j=0}^{L}(-i)^{j}cos^{L-j}\theta_{\lambda}sin^{j}\theta_{\lambda}\hat{\Phi}_{L,j}
\end{align*}
        に代入して偶奇性、及び虚数の出方を整理すれば自明にもとまる。
        \item 2について\
        \begin{align*}
            W_{\phi_{i}}(\theta=0)=I=A(1)I+B(1)\sigma_{z}
        \end{align*}
        故に、$A(1)=1,B(1)=0$
        \item 4について\\
        Wはユニタリでなければならないため、$W^{\dagger}W=1$
        ここにWの具体系を代入するともとまる。
    \end{enumerate}
\end{proofbox}

十分条件の証明は、(1)~(5)を満たす与えられた式から$\vec{\phi}$を逆算するアルゴリズムを、具体的に示すことによって与えられる。以下、そのアルゴリズムを紹介する。
\begin{setupbox}{(A,B,C,D)のから$\phi$を求めるアルゴリズム}
INPUT:条件1から条件4を満たす(A,B,C,D)及びそのL次べき展開
\begin{align*}
    A(\lambda)&=\sum_{n=0}^{L}a_{n}x^{n}\quad
    B(\lambda)=\sum_{n=0}^{L}b_{n}x^{n}\\
    C(\lambda)&=\sum_{n=0}^{L}c_{n}x^{n} \quad
    D(\lambda)=\sum_{n=0}^{L}d_{n}x^{n}
\end{align*}
求める関数：$W_{\vec{\phi}}[\theta_{\lambda}]$
\begin{align*}
        W_{\vec{\phi}}[\theta_{\lambda}]
        &=W_{\phi_{1}}W_{\phi_{2}}\cdots W_{\phi_{L}}\\
        &=\sum_{j=0}^{L}cos^{L-n}\theta_{\lambda}sin^{n}\theta_{\lambda}\Phi_{L,n}\\
        &=\begin{cases}
            A(x)I+B(x)\sigma_{z}+C(y)\sigma_{x}+D(y)\sigma_{y}\quad(L\in odd)\\
            A(x)I+B(x)\sigma_{z}+xC(y)\sigma_{x}+xD(y)\sigma_{y}\quad (L\in even)
        \end{cases}
    \end{align*}
\begin{enumerate}
    \item $(A,B,C,D)$から$\Phi_{L,j}$を逆引き
        \begin{align*}
        \Phi_{L,j}=
        \begin{cases}
            i^{j}\sum_{n=0}^{L-j}(a_{n}+ib_{n}){}_{\frac{L-n}{2}}C_{\frac{j}{2}}\quad (j\in even)\\
            i^{j}\sum_{n=0}^{L-j}(ic_{n}-id_{n}){}_{\frac{L-n}{2}}C_{\frac{j-n}{2}}\quad (j\in odd)
        \end{cases}
        \end{align*}
        ただし、${}_{i}C_{j>i}=0$とする。
    \item $\Phi_{L,j}$から$\phi_{L}$及び$\Phi_{L-1,j}$を逆引き
    \begin{align*}
        \Phi_{L-1,j}=\sum_{k=0}^{j}\begin{cases}
            -e^{-i(-1)^{j}\phi_{L}}\Phi_{L,k}\quad (j+k\in odd)\\
            \Phi_{L,k}\quad (j+k\in odd)
        \end{cases}
    \end{align*}
    及び、
    \begin{align*}
        e^{i\phi_{L}}
        =\frac{\sum_{k=1,odd}^{L}\Phi_{L,k}}{\sum_{k=1,even}^{L}\Phi_{L,k}}
        =\frac{c_{2[\frac{L}{2}]-1}+id_{2[\frac{L}{2}]-1}}{(-1)^{[\frac{L}{2}]}(a_{L}+ib_{L})}
    \end{align*}
    \item $\Phi_{i,j}$から$\phi_{i}$及び$\Phi_{i-1,j}$を逆引き
     \begin{align*}
        \Phi_{i-1,j}=\sum_{k=0}^{j}\begin{cases}
            -e^{-i(-1)^{j}\phi_{L}}\Phi_{i,k}\quad (j+k\in odd)\\
            \Phi_{i,k}\quad (j+k\in odd)
        \end{cases}
    \end{align*}
    \begin{align*}
        e^{i\phi_{i}}
        =\frac{\sum_{k=1,odd}^{i}\Phi_{i,k}}{\sum_{k=1,even}^{i}\Phi_{i,k}}
    \end{align*}
    \item $\phi_{1}$が出てくるまで繰り返す。最後に$\Phi_{0,0}=1$が出れば完了。
\end{enumerate}
OUTPUT:$(\phi_{1}\cdots \phi_{L})$
\end{setupbox}
\begin{proofbox}
    以下では、このアルゴリズムにより、実際にQSP漸化式や境界条件を保ちつつ($\phi_{1}\cdots \phi_{L}$)が取り出せることをみる。\\
    \\
    ただし、ここではLが偶数の場合についてのみ、上記が正しいことを示す。
    \begin{enumerate}
        \item 1の逆引きについて\\
        ここは次数の比較により直接行える。\\
        例えば(A,B)部分について、Wの展開のうち、A,Bに相当するのはjが偶数である部分なので、
        \begin{align*}
            W|_{\text{(A,B)部分}}=\sum_{j\in even}(-i)^{j}x^{L-j}(1-x)^{\frac{j}{2}}(Re(\Phi_{L,j})+iIm(\Phi_{L,j})\sigma_{z})
        \end{align*}
        一方、(A,B)のべき展開は、
        \begin{align*}
            A(x)I+iB(x)\sigma_{z}=\sum_{n\in even}(a_{n}+ib_{n}\sigma_{z})x^{n}
        \end{align*}
        この時、以下の恒等式を用いる。
        \begin{align*}
            x^{n}&=x^{n}(x^{2}-(1-x^{2}))^{\frac{L-n}{2}}\\
            &=\sum_{k=0}^{\frac{L-n}{2}}{}_{\frac{L-n}{2}}C_{k}x^{L-2k}(1-x^{2})^{k}
        \end{align*}
        故に、
        \begin{align*}
            A(x)I+iB(x)\sigma_{z}=\sum_{n\in even}\sum_{k=0}^{\frac{L-n}{2}}{}_{\frac{L-n}{2}}C_{k}x^{L-2k}(1-x^{2})^{k}(a_{n}+ib_{n}\sigma_{z})
        \end{align*}
        ここで、j=2kと変数を取り替えると、
        \begin{align*}
            A(x)I+iB(x)\sigma_{z}=\sum_{n\in even}\sum_{j=0,even}^{L-n}{}_{\frac{L-n}{2}}C_{\frac{j}{2}}x^{L-j}(1-x^{2})^{\frac{j}{2}}(a_{n}+ib_{n}\sigma_{z})
        \end{align*}
        添字の和は以下のように交換できる。
        \begin{align*}
            \sum_{n\in even}\sum_{j=0,even}^{L-n}=\sum_{j\in even}\sum_{n=0,even}^{L-j}
        \end{align*}
        故に、(A,B)のべき展開は、
        \begin{align*}
            A(x)I+iB(x)=\sum_{j\in even}x^{L-j}(1-x^{2})^{\frac{j}{2}}(\sum_{n=0,even}^{L-j}{}_{\frac{L-n}{2}}C_{\frac{j}{2}}(a_{n}+ib_{n}\sigma_{z}))
        \end{align*}
        実部、虚部ごとに係数比較し、まとめると、
        \begin{align*}
            \Phi_{L,j}=i^{j}\sum_{n=0}^{L-j}(a_{n}+ib_{n}){}_{\frac{L-n}{2}}C_{\frac{j}{2}}\quad (j\in even)
        \end{align*}
        また、jが奇数の時にも同様のことを行うと、
        \begin{align*}
            \Phi_{L,j}=i^{j}\sum_{n=0}^{L-j}(ic_{n}-id_{n}){}_{\frac{L-n}{2}}C_{\frac{j-n}{2}}\quad (j\in odd)
        \end{align*}
        \item 2の逆引きについて\\
        QSP漸化式を用いて逆引きしていく。まず、QSP漸化式を持ちると、
        \begin{align*}
    \Phi_{L,j}
    &= \Phi_{L-1,j}
    + e^{i(-1)^{j+1}\phi_L}\Phi_{L-1,j-1},\\
    \Phi_{L,j-1}
    &= \Phi_{L-1,j-1}
    + e^{i(-1)^j\phi_L}\Phi_{L-1,j-2},\\
    &\ \vdots\\
    \Phi_{L,1}
    &= \Phi_{L-1,1}
    + e^{i\phi_L}\Phi_{L-1,0},\\
    \Phi_{L,0}
    &= \Phi_{L-1,0}.
\end{align*}
故に、右辺を連鎖的に消していくと、
\begin{align*}
        ...
        \end{cases}
\end{align*}
また、境界条件$\Phi_{L-1,L}=0$を満たすようにするには、QSP漸化式から、
\begin{align*}
    \Phi_{L,L}=\Phi_{L-1,L}+e^{i(-1)^{L+1}\phi_L}\Phi_{L-1,L-1}=e^{-i\phi_L}\Phi_{L-1,L-1}
\end{align*}
を満たすように$\phi_{L}$を選ばないとダメ。これを満たすには、上記の$\Phi_{L-1,j}$の具体系から、
\begin{align*}
    e^{i\phi_{L}}=\frac{\sum_{k\in odd}^{L}\Phi_{L,k}}{\sum_{k\in even}^{L}\Phi_{L,k}}
\end{align*}
となる必要がある。
\item 3について\\
これはLをL-1に、L-1をL-2に置換して2を行えばそれでいい。
\item 4について\\
\begin{align*}
            W_{\vec{\phi}}(\theta=0)=\sum_{j}1^{L-j}0^{j}\hat{\Phi}_{L,j}=\hat{\Phi}_{L,0}=I \iff \Phi_{L,0}=1
\end{align*}
故に、$\Phi_{0,0}=1$の必要十分条件が$A(1)=1,B(1)=0$であるため、これを満たすなら本アルゴリズムは正常に終了する。
\end{enumerate}
\end{proofbox}
以上の手続きにより、組 $(A,B,C,D)$ が所定の条件を満たす場合には、それらに対応する QSP を具体的に構成することが可能となった。\\
\\
しかし、実際にQSPを適用する上では、これだけでは十分ではない。\\
実際の応用では、前節で見たように、補助量子ビットに対する post-selection を通じて、最終的な変換から $(A,B)$ などの一部の成分のみを取り出して利用することが多い。そのため、実際の応用では $(A,B,C,D)$のすべてがあらかじめ指定されるとは限らず、例えば $(A,B)$ や $(A,C)$ のみが所望の関数として与えられる場合を考える必要がある。\\
\\
そこで次に、$(A,B,C,D)$ の一部のみが指定された場合に、それらを含むQSPが実装可能であることの必要十分条件を考える。例えば、(A,B)のみの場合は以下の通りである。
\begin{resultbox}{(A,B)のQSP実装条件}
    (A,B)が与えられた際、QSPでそれが実装可能である必要十分条件は、以下を満たすことである。
    \begin{enumerate}
        \item $|x|\leq 1$において、
        \begin{align*}
            A(x)^{2}+B(x)^{2}\leq 1
...
\end{resultbox}
\begin{proofbox}
    十分条件は後に構成アルゴリズムを与えることで示す。
    ここでは必要性のみを示す。

    $W$のユニタリ性より、
    \begin{align}
        A(x)^2+B(x)^2
        +\alpha(x)\left(C(y)^2+D(y)^2\right)=1,
        \qquad
        y^2=1-x^2,
        \label{eq:AB-unitarity}
    \end{align}
    が成り立つ。ただし、
    \begin{align*}
        \alpha(x)=
        \begin{cases}
            1 & (L\in\mathrm{odd}),\\
            x^2 & (L\in\mathrm{even})
        \end{cases}
    \end{align*}
    とした。また、$C,D$はいずれの場合も実係数の奇多項式である。

    \begin{enumerate}
        \item $|x|\leq1$のとき、$y=\sqrt{1-x^2}$は実数である。
        したがって
        \begin{align*}
            \alpha(x)\left(C(y)^2+D(y)^2\right)\geq0
        \end{align*}
        であり、Eq.~\eqref{eq:AB-unitarity}より
        \begin{align*}
            A(x)^2+B(x)^2\leq1.
        \end{align*}
        ...
        \end{align*}
        は純虚数である。
        $C,D$は実係数の奇多項式なので、$C(y),D(y)$も純虚数となり、
        \begin{align*}
            C(y)^2+D(y)^2\leq0.
        \end{align*}
        また$\alpha(x)\geq0$であるから、
        Eq.~\eqref{eq:AB-unitarity}より
        \begin{align*}
            A(x)^2+B(x)^2\geq1.
        \end{align*}

        \item $L$が偶数の場合、Eq.~\eqref{eq:AB-unitarity}は
        \begin{align*}
            A(x)^2+B(x)^2
            +x^2\left(C(y)^2+D(y)^2\right)=1
        \end{align*}
        である。ここで$x\to ix$とすると、
        $y^2=1+x^2$であるから
        \begin{align*}
            A(ix)^2+B(ix)^2
            -x^2\left[
                C\!\left(\sqrt{1+x^2}\right)^2
                +D\!\left(\sqrt{1+x^2}\right)^2
            \right]
            =1.
        \end{align*}
        $C,D$は実係数多項式であるため、$x\geq0$において角括弧内は非負である。
        したがって
        \begin{align*}
            A(ix)^2+B(ix)^2\geq1.
        \end{align*}
...
十分条件の証明として、(A,B)から$(\phi_{i})$の復元アルゴリズムを構築することを行う。しかしこの際、$(A,B,C,D)$から$(\phi_{i})$を復元する方法は既に構築したので、これを行うためには$(A,B)$から$(A,B,C,D)$を構築できればそれで十分である。故に、その方法を構築する。
\begin{setupbox}{(A,B)から(A,B,C,D)を復元するアルゴリズム}
INPUT:条件を満たす(A,B)
    \begin{enumerate}
    \item $F(x)=1-A(x)^2-B(x)^2$を計算する。
    \item $y^2=1-x^2$を用いて
    \begin{align*}
        G(y)=
        \begin{cases}
            ...
            &(L\in\mathrm{odd}),\\[1mm]
            \displaystyle
            \frac{F(\sqrt{1-y^2})}{1-y^2}
            &(L\in\mathrm{even})
        \end{cases}
    \end{align*}
    と定義する。
    \item $G(y)$を複素数体上で因数分解する。
    \item 複素共役な零点の各組から一方を選び、
    \begin{align*}
        Q(y)=\gamma\prod_k(y-z_k)
    \end{align*}
    が
    \begin{align*}
        G(y)=Q(y)Q(y)^*
    ...
    とする。
\end{enumerate}
OUTPUT:QSP実装条件を満たす(A,B,C,D)
\end{setupbox}
\begin{proofbox}
    上記のアルゴリズムによって構成された$(A,B,C,D)$が、
    QSP実装条件を満たすことを示す。
    ...
    \end{align*}
    とする。
    $A,B$は同一のパリティを持つため、$F(x)$は偶多項式である。
    したがって、$y^2=1-x^2$を用いることで、
    $F$は$y^2$の多項式として書き直すことができる。

    まず$L$が奇数の場合を考える。このとき
    \begin{align*}
        G(y)=F\!\left(\sqrt{1-y^2}\right)
    \end{align*}
    とする。
    $0\leq y\leq1$では$x=\sqrt{1-y^2}\in[-1,1]$であるから、
    条件1より
    \begin{align*}
        G(y)=1-A(x)^2-B(x)^2\geq0.
    \end{align*}
    一方、$|y|\geq1$では
    \begin{align*}
        x=i\sqrt{y^2-1}.
    \end{align*}
    $L$が奇数の場合、$A,B$は実係数の奇多項式であるから
    $A(ix),B(ix)$は純虚数であり、
    \begin{align*}
        A(ix)^2+B(ix)^2\leq0.
    \end{align*}
    したがって、この場合にも$G(y)\geq0$である。
    よって$G$は実軸上で非負な実多項式となる。
    ...
    \begin{align*}
        ...
        =
        \frac{
        F\!\left(\sqrt{1-y^2}\right)
        }{1-y^2}
    \end{align*}
    とする。
    $|y|<1$では条件1より分子、分母ともに非負であるから
    $G(y)\geq0$である。
    一方、$|y|>1$では
    \begin{align*}
        x=i\sqrt{y^2-1}
    ...
    \end{align*}
    として現れる。
    また、実零点は$G(y)\geq0$より必ず偶数重根である。
    したがって、それぞれの複素共役な零点の組から一方を選び、
    実零点についてはその重複度の半分を選ぶことで、
    ある複素係数多項式$Q(y)$を
    \begin{align*}
        Q(y)=\gamma\prod_k(y-z_k)
    \end{align*}
    ...
    さらに、$G$は$y$について偶多項式であり、
    $y=0$に必要な零点を$Q$へ割り当てた上で
    $\pm z$の零点を対称に選ぶことで、
    $Q$を奇多項式として取ることができる。
    したがって、その実部、虚部である$C,D$も
    実係数の奇多項式として取ることができる。

    以上より、構成された$(A,B,C,D)$は
    QSP実装条件を満たす。
\end{proofbox}
最後に、(A,B)ではなく、よりいろんな論文で取り上げられている仮定である、(A,C)が与えられた場合に、QSPが実装できる条件、及びアルゴリズムを、証明はしないが与えておく。ここで、notationを少し変更し、Qubitizationのものを採用することにする。
\begin{resultbox}{$(A,C)$のQSP実装条件}
    $(A,C)$が与えられた際、それを含むQSPが実装可能であるための
    必要十分条件は、以下を満たすことである。
    \begin{enumerate}
        \item $A$が余弦関数の有限和として
        \begin{align*}
        ...
        \item 任意の実数$\theta$に対して
        \begin{align*}
            A(\theta)^2+C(\theta)^2\leq1.
        \end{align*}
    \end{enumerate}
\end{resultbox}
\begin{setupbox}{$(A,C)$から$(A,B,C,D)$を構成するアルゴリズム}
    INPUT：上記のQSP実装条件を満たす$(A,C)$

    \begin{enumerate}
        \item 以下の非負な三角多項式を定義する。
        \begin{align*}
            F(\theta)
            :=
            1-A(\theta)^2-C(\theta)^2.
        ...
        \item $z=e^{i\theta}$とおき、$F(\theta)$をLaurent多項式
        $F(z)$として表す。

        \item $F(z)$を複素数体上で因数分解し、
        \begin{align*}
            F(\theta)
            =
        ...
        を満たす多項式$Q$を構成する。
        ただし、零点は複素共役性および
        $z\leftrightarrow z^{-1}$の対称性を保つように選択する。

        \item $Q(e^{i\theta})$を実部と虚部に分解し、
        \begin{align*}
            Q(e^{i\theta})
            =
            B(\theta)+iD(\theta)
...
\end{align*}
故に、Block-encodingを使った関数の実装は、より一般の行列でも同様の手続きで行える。この一連の一般化した手続きを、量子特異値変換(QSVT)と呼ぶ\cite{Gilyen:2018khw}。\\
\\
...
\\
ただし、一般的なオラクルの形は、当然ながら関数の形に依存するため、そこに触れることなしにreflectionの構成を議論することは難しい。故に、この説では、一般論ではなく、特定の問題に対してのGrover回路をQSVTを用いて構築することで、QSVTを用いてGroverを再現できる可能性を示すにとどめることにする\footnote{ただし、ここでいうQSVTとは実を言うとQSPのことである。QSVTの場合、全ての特異値が正であるため、reflectionが自明になってしまう。}。\\
\\
Groverのアルゴリズムとして、以下のような問題を考える。
\begin{problembox}{問題のセットアップ}
    行列の固有値を考えた際、その固有値が正となるような固有ベクトルを求めよ。
\end{problembox}
この問題において、reflectionというのは、以下のように書き換えることが可能である。
\begin{align*}
    f(\lambda)\ket{\lambda}\bra{\lambda}=sign(\lambda)\ket{\lambda}\bra{\lambda}
\end{align*}
故に、Groverのアルゴリズムを上記の問題において構成できるかどうかは、sign関数を多項式近似で実装できるかに全てがかかっている。この実装可能性の答えは、以下のような性質で与えられる。
\begin{theorembox}{sign関数の実装可能性}
符号関数は、誤差率$\epsilon\in [0,\frac{1}{2}]$,幅$\delta>0$とした時、次数$L=O(\frac{1}{\delta}log\frac{1}{\epsilon})$の下で以下のように関数近似できる。
\begin{enumerate}
    \item $x\in[-2,2]$に対し、$|P(x)|\leq 1$\\
    \item $x\in\frac{[-2,2]}{(-\delta,\delta)}$に対し、$|P(x)-sign(x)|\in \epsilon$
\end{enumerate}
\end{theorembox}
\begin{proofbox}
    
\end{proofbox}
sign関数は符号のみを返すアルゴリズムであるため、確かにreflectionである。故に、固有値の正負を分ける問題においては、確かに上記のsign関数を実装することが、Groverを実装することにつながる。

\subsection{新しいアルゴリズムの開発}
以下では、QSVTにインスパイアされ提案された、いくつかの新しいアルゴリズムを紹介する。\\
ただし、以下ではBlock-encodingの規格化、及びAncillaのサイズが非自明となるような拡張を考えることが多い。このnotationを整理するために、以下のような記法を導入する。
\begin{defbox}{Block-encodingの記法}
    ある行列Aの、以下を満たすBlock-eoncoding Uが与えられたとする。
    \begin{enumerate}
        \item Aの規格化定数が$\alpha$である。
        \item ancillaの数がm個である。
        \item Block-eoncodinが厳密にAを与えるのではなく、限りなくAに近い行列を与えるものとして考える。すなわち、
        \begin{align*}
            |A-\bra{0}^{\otimes m}U\ket{0}^{\otimes m}|\leq \epsilon
        \end{align*}
    \end{enumerate}
    この時、Uのことを、$(\alpha,m,\epsilon)$-block encodingと呼ぶことにする。
\end{defbox}
\subsubsection{基底準備}
例えば、QSVTを用いることにより、断熱近似とは異なる手法で基底状態を用意する方法が近年提示されている\cite{Lin:2020zni}。アイデアとしては、ただreflectionを構成してGroverを行うだけである。従来ではこのreflectionを構成することが困難であったが、QSVTを用いることで上記のrefectionを構成できるようになったため、このセットアップが具体的に実現できるようになった。以下では、このオラクルの構築方法について議論を行う。\\
\\
鍵となる定理を与える前に、「$(\beta,b,\epsilon_{1})$-state preparation pair」というものを準備しておく。
\begin{defbox}{$(\beta,b,\epsilon_{1})$-state preparation pair}
    ある実数ベクトル$y_{j}$に対して、$|y|\leq \beta$とする。\\
    \\
    ユニタリ変換の組$(P_{L},P_{R})$が$(\beta,b,\epsilon_{1})$-state preparation pairであるということは、以下を満たすことである。
    \begin{enumerate}
        \item b量子ビットの初期状態への応答が以下の通りである\footnote{ただし、$2^{b}>j$である。}。
        \begin{align*}
            P_{R}\ket{0}^{\otimes b}=\sum_{j=0}^{2^{b}-1}c_{j}\ket{j}\\
            P_{L}\ket{0}^{\otimes b}=\sum_{j=0}^{2^{b}-1}d_{j}\ket{j}
        \end{align*}
        \item $\sum_{j=0}^{m-1}|\beta(c_{j}^{\star}d_{j})-y_{j}|\leq \epsilon_{1}\equiv$
        \item $j\in [m,2^{b}-1]$に対して、$c_{j}^{\star}d_{j}=0$\footnote{ただし、これは縮約和を取っていないことに注意。}
    \end{enumerate}
\end{defbox}
オラクルの構成において、カギとなる定理は以下の通りである。
\begin{theorembox}{Block-encodingの加法}
    行列$A_{j}$の$(\alpha,m,\epsilon_{2})$-Block-encoding $U_{j}$が与えられたとする。\\
    この時、
    \begin{enumerate}
        \item $(\beta,b,\epsilon_{1})$-state preparation pair $(P_{L},P_{R})$
        \item b量子ビット分のancilla
    \end{enumerate}
    を用いることで、$A\equiv \sum_{j}y_{j}A_{j}$の$(\alpha\beta,m+b,\alpha\epsilon_{1}+\alpha\beta\epsilon_{2})$-Block encodingが構成可能である。
\end{theorembox}
\begin{proofbox}
    まず、以下のようなユニタリ行列Wを定義する。
    \begin{align*}
        W=\sum_{j=0}^{m-1}\ket{j}\bra{j}_{b}\otimes U_{j}+(I-\sum_{j=m}^{2^{b}-1}\ket{j}\bra{j}_{b})\otimes I
    \end{align*}
    このWの前後で、state preparation pair及びその逆行列を作用させた以下の行列を定義する。
    \begin{align*}
        \tilde{W}\equiv P_{L}^{\dagger}WP_{R}
    \end{align*}
    この時、$\tilde{W}$をpost-selectionしたものが、Block-encodingの加法に対応している。この事実は以下のように理解できる。
    \begin{align*}
        &|A-\alpha\beta\bra{0}^{\otimes b}\bra{0}^{\otimes a}W\ket{0}^{\otimes b}\ket{0}^{\otimes a}|\\
        =&|A-\alpha\beta \sum_{j=0}^{m-1}\sum_{j'=0}^{m-1}c_{j}^{\star}d_{j'}\bra{j}_{b}\bra{0}^{\otimes a}W\ket{j'}_{b}\ket{0}^{\otimes a}|\\
        =&|A-\alpha\sum_{j=0}^{m-1}\beta(c_{j}^{\star}d_{j})\bra{0}^{\otimes a}U_{j}\ket{0}^{\otimes a}|\\
        \leq&\alpha\epsilon_{1}+\alpha\sum_{j}y_{j}|A_{j}-\bra{0}^{\otimes a}U_{j}\ket{0}^{\otimes a}|\\
        \leq& \alpha\epsilon_{1}+\alpha\beta\epsilon_{2}
    \end{align*}
    故に、state-preparation pair及びWを構築できたとすると、Block-encodingの加法が構築可能である\footnote{Wの構築はそこまで難しくない。適当なUに対してコントロールを実装するだけで済むからである。しかし、state-preparation pairは対応する線形結合次第なので、そんなに自明ではない。}。
\end{proofbox}
上記の定理を用いると、ハミルトニアンHに対して、例えば単位行列を足したもの、すなわち$H'=H-\mu I$のBlock-encodingが可能となることを意味する。これは、エネルギー固有値のシフトを導入できたことを意味する。これを用いて、例えば基底エネルギーのみマイナス符号にすることを考えることが出来る。\\
\\
一方、行列の固有値の符号だけを抜き出す、sign関数を導出できることは、Groverの欄で見たとおりである。\\
これらの事実を組み合わせると、以下のようなセットアップで、基底状態のみを反転させるreflection、及び基底準備を導出できる。
\begin{setupbox}{QSVTによる基底準備アルゴリズム}
\begin{enumerate}
    \item ハミルトニアンのBlock-encodingを導入
    \item Block-encodingの加法により、エネルギーシフトを導入
    \item エネルギーシフト後のBlock-encodingに対して、sign関数を導入。これが実質的なGroverのreflectionとなる。
    \item Groverのアルゴリズムより、基底準備が完了
\end{enumerate}
\end{setupbox}
...
\subsubsection{エラー伝播のない(transversal)演算}
上記のエラー伝播の本質は、「CNOTを通じてエラーが拡大すること」であった。故に、エラー伝播のない誤り訂正符号を構築しようと思うなら、「CNOTゲートができる限り１対１となる」ような訂正符号を構築すればいいであろう。このような回路における量子演算を、「transversalな演算」と呼ぶ。\\
\\
...
上の節では、エラー耐性を持つ量子演算が、ずっとエラー耐性を持ち続けるための条件を述べた。しかし、それより前に、Gittesmann-Knillの定理より、Cliffordゲートを用いた、要するにエラー耐性を持つ量子演算だけの量子コンピュータは、何ら量子優位性を持たないことを示した。\\
この定理の存在により、真に量子優位性を持つ計算をFault-tolerantに行うためには、えら-体制の無い、すなわちClifford群に属さないゲートを、エラー訂正できるように実装する方法を考えねばならない。
\\
...
\begin{resultbox}{CliffordゲートUのエラー訂正可能性}
    ...
    この時、状態操作後のstateに対して、誤り検出回路を構築可能である。
\end{resultbox}
他の教科書ではあまりちゃんと強調しないが、あくまでこれは十分条件である。故に、実はT-ゲートをエラー訂正可能性を保ったまま実装する方法を行うことは、「理論上まだ不可能ではない」、ということが告げるべき事象である。\\
\\
...
\end{setupbox}
ここで、良い知らせと悪い知らせが1つずつ存在する。\\
悪い知らせとは、以下のstatementが成立することである。
\begin{theorembox}{Eastin-Knillの定理}
    全てのユニバーサルゲートを、「transversalに」実行できる誤り訂正符号は存在しない。
\end{theorembox}
\begin{proofbox}
    すまんが原論文みてくれ
\end{proofbox}
Cliffordゲートにはtransversalな拡張が知られていることが多い。故に、このstatementは、ロジカルTゲートを実装したければ、transversal性を捨てねばならないことを、多くの場合に意味する\footnote{ただし、別に捨てたければCliffordゲートのtransversal性を捨てても良い。あまりメリットはないらしいが}。\\
\\
一方、良い知らせとは、2006年ごろの研究により、ロジカルTゲートの実装過程で一時的にtransversal性を失ったとしても、それ自体は必ずしも致命的ではないことが明らかとなったことである。\\
この考え方に基づく代表的な手法が「magic state injection」及び「magic state蒸留」である。以下でその概要を紹介する。\\
\\
なお、この方法は途中でtransversalの構造を捨てており、Estin-Knillの定理の制限を逃れているため、量子優位性の必要条件は満たしているということを最初に明記しておく。それでいて尚且つ、誤り訂正の構造そのものは保っているのが、このアイデアの面白いところである。
...
以下では、magic-stateを用意する方法を考える。しかし、Fault-tolerantに直接生成する方法は、Estin-Knillの定理より禁止されている。「magic-state蒸留」は、エラーをある程度許容しつつ、質の高いmagic stateを大量に用意する方法である。\\
\\
...
\begin{setupbox}{5ビットのmagic-state蒸留}
 ...
 \end{align*}
    \begin{enumerate}
        \item 初期状態の用意\\
        第一段階では、「transversal性を捨てることを許容してT-ゲートを用意する」ことを考える。この段階でEstin-Knillの制約からは逃れている。\\       \\
        ...
        \end{align*}
    \end{enumerate}
以上で用いた操作はパウリ群の観測とCliffordゲートのみである。故に、最初のエラー許容のステップを除くと、すべての操作はTransversalである。
\end{setupbox}
\begin{proofbox}
...
\begin{enumerate}
    ...
    \end{align*}
    $\sum_{x}|a_{x}^{2}|,|b_{x}^{2}|$の値は、以下の恒等式から理解できる。
    \begin{align*}
    ...
    \\
    まず、「規格化された測定後状態」にキチンと修正する。規格化は測定確率pで状態を割ればいいので、規格化した測定後状態は以下のように記述される。
    \begin{align*}
...
\begin{resultbox}{Filtering回路}
    ...
    Gの計算量：\\
    \begin{align*}
        \text{(初期状態を用意するクエリの作用回数)}&: O(\frac{1}{\gamma})\\
        \text{(Block encodingのクエリの作用回数)}&:O(\frac{\alpha}{\gamma\delta}log(\frac{1}{\gamma\epsilon}))\\
        \text{(その他のゲート)}&:O(\frac{m\alpha}{\gamma\delta}log(\frac{1}{\gamma\epsilon}))
    \end{align*}
\end{resultbox}
...
\\
断熱オンリーのアルゴリズムを行う場合をまず考える。忠実度が$1-\epsilon$である条件を考えると、これはKZ機構の解析の結果から、
\begin{align*}
    1-\epsilon = 1- exp (\frac{T}{N^{2}}) \iff T=O(N^{2}long\frac{1}{\epsilon})
\end{align*}
また、時間発展回路自体もTrotter近似を用いているため、その分割分の誤差が存在する。この誤差を$\epsilon_{tro}$程度に抑えるために分割数を$O(T\frac{1}{\epsilon_{tro}})$とする。時間発展1step分の回路の計算量はO(N)であるため、
\begin{resultbox}{断熱オンリーの場合の必要計算量}
\begin{align*}
    \text{(断熱準備の計算量)}=O(N)\times O(T\frac{1}{\epsilon_{tro}})=O(N^{3}log\frac{1}{\epsilon}\frac{1}{\epsilon_{tro}})
\end{align*}
\end{resultbox}
次に、ハイブリットアルゴリズムにおける計算量を考える。
hybridアルゴリズムの計算量は、大まかに分けて初期状態の用意とフィルタリングの部分に分けられるため、まず以下のような形になる。
\begin{align*}
    \text{(hybridの計算量)}=\text{(初期状態用意クエリ計算量)}+\text{(Filtering計算量)}
\end{align*}
Filteringの方で初期状態用意のクエリを複数回用いるため、主要な寄与は後者である。故に、前者は無視していい。Filtering回路の計算量を求める公式を代入すると、計算量は
\begin{align*}
    \text{(hybridの計算量)}
    &=\text{(Filtering計算量)}\\
    &=\text{(初期状態用意クエリ計算量)}\times O(\frac{1}{\gamma})\\
    &+\text{(Block-encodingクエリ計算量)}\times O(\frac{\alpha}{\gamma\delta}log(\frac{1}{\gamma\epsilon}))\\
    &+O(\frac{m\alpha}{\gamma\delta}log(\frac{1}{\gamma\epsilon}))\\
\end{align*}
これの計算量のスケーリングを把握するために、クエリ計算量、$\gamma$などのスケーリングを考える。\\
まず、Block-encodingクエリの計算量であるが、今の場合ハミルトニアンがLCUの形をしているため、大体以下の計算量で用意できることが知られている。\\
\begin{align*}
    \text{(Block-encodingクエリ計算量)}=O(N)
\end{align*}
次に、初期状態用意クエリの計算量を考えるが、問題設定から、断熱時間はパラメータとして与えられる。これが大きいほどFilteringが楽になり、
小さいほどFilteringに時間がかかることがFiltering回路の計算量と断熱クエンチの結果から直感的に理解できる。このため、$T=O(N^{a})$とこちらで与えたとすると、トロッター誤差と、時間発展1step分の回路がO(N)であることを踏まえると、計算量は以下の通りとなる。
\begin{align*}
    \text{(初期状態用意クエリ計算量)}=O(N^{1+a}\frac{1}{\epsilon_{tro}})
\end{align*}
最後に、$\gamma$の値を与える。これはKZ機構の解析において、$N>>1$として、$P_{GS}$の積分を評価することで求められる。そのために、積分を以下の形に分ける。
\begin{align*}
...
故に、$\gamma$のスケーリングは、
\begin{align*}
    \gamma =O(exp(-N^{1-\frac{a}{2}}))
\end{align*}
となる。残りの定数は、問題設定から全てO(1)であるため無視していい。結局$T=O(N^{a})$の際のハイブリットの計算量は、
\begin{resultbox}{$T=O(N^{a})$のハイブリットアルゴリズムの計算量}
    \begin{align*}
    \text{(hybridの計算量)}=&O(N^{1+a}exp(N^{1-\frac{a}{2}})\frac{1}{\epsilon_{tro}}) \\
    &+ O(N(N^{1-\frac{a}{2}}+log\frac{1}{\epsilon})exp(N^{1-\frac{a}{2}}))\\
    &+ O((N^{1-\frac{a}{2}}+log\frac{1}{\epsilon})exp(N^{1-\frac{a}{2}}))
\end{align*}
\end{resultbox}
当然、aの値を調整して最適な計算量を求めることもできる。今の場合、saddle-pointは$a=2$付近であると推測できるため、主寄与は第1項である。故に、第1項をaで微分して0になるようなaの値を推定すると、それが(大体の)最良計算量になる。第一項をaで微分すると、
\begin{align*}
    &\frac{d}{da}(N^{1+a}exp(N^{1-\frac{a}{2}}))\\
    =& logN\times N^{1+a}exp(N^{1-\frac{a}{2}})(1-\frac{1}{2}N^{1-\frac{a}{2}})\\
    \iff& a=2-\frac{log2}{logN}
\end{align*}
この際、
\begin{align*}
    &N^{1+a}=N^{3}(N^{\frac{1}{logN}})^{2log2}=N^{3}(e^{\frac{logN}{logN}})^{2log2}=O(N^{3})\\
    &N^{1-\frac{a}{2}}=O(1)
\end{align*}
故に、最良計算量は以下のようであると推定される。
\begin{resultbox}{ハイブリットアルゴリズムの最良計算量}
    \begin{align*}
        O(N^{3}\frac{1}{\epsilon_{tro}})+O(Nlog\frac{1}{\epsilon})
    \end{align*}
\end{resultbox}
誤差依存性に関しては確かに掛け算が足し算になったくらいの改善が見られた。スケーリングはexpが重すぎるため、改善の余地が見られなかった。モデル依存？

\subsubsection{Non-crossing定理}
このアイデアの活かしどころとして、有限系でギャップレスが存在する場合なら良さそうであると述べたが、断熱のような状況を考える限り、それが非常に厳しいことが、von-Neumannによって示された。以下では、その概要について述べる。\\
\\
statementは以下の通りである。
\begin{theorembox}{Non-crossing定理}
    断熱定理のような1-パラメータで制御されるハミルトニアンでは、追加で何かしらの構造がない限り、Gapplessが生じない。
\end{theorembox}
\begin{proofbox}
    簡単のため、2準位系のハミルトニアンを考える。一般のハミルトニアンでも、基底と第一励起以外を固有状態に基底を取り替えると、このようなハミルトニアンを記述可能である。\\
    \\
    この場合、エルミート性から、ハミルトニアンは以下の通りである。
    \begin{align*}
        H=\begin{pmatrix}
            E_{1} & V\\
            V^{*} & E_{2}\\
        \end{pmatrix}
    \end{align*}
    この行列の固有値は、たかだか$2\times 2$行列の計算から、以下のようになる。
    \begin{align*}
        E_{\pm}=\frac{E_{1}+E_{2}}{2}\pm \sqrt{(\frac{E_{1}-E_{2}}{2})^{2}+|V|^{2}}
    \end{align*}
    故に、ギャップが閉じている必要十分条件は、
    \begin{align*}
        E_{1}=E_{2}\quad and \quad V=0
    \end{align*}
    である。\\
    \\
    今、ハミルトニアンが１パラメータにより支配されているとする。この場合、上記の２条件のうち、満たすことができるのは精々$E_{1}=E_{2}$の条件までである。一般には額各校までは消せないので、Gapplessが生じない。
\end{proofbox}
逆に、縮退があることがわかっている場合などは、非対角項が生じないため、この定理から逃れられる。\\
\\
で、ここから重要な話として、「縮退がある場合はcrossingが真に起きうるため、hybridが存在するのではないか」という発想が出現する。ただし、この発想はおそらくダメである。なぜなら、crossing周りにおいて、非対角項が0となるため、基底と第一励起を入れ替えた後、基底に映るような機構がそもそも存在しないためである。

コメントとFuture-work
\begin{enumerate}
    \item Q.Groverの計算する際に最良計算量やらにゃ正しい答え得られない気がするけど、そこんとこどうなの？\\
    A.普通のGroverの奴と同じような形で位相から計算量を求められるので問題ない。
    \item Q.出てくる状態はcat-stateになる気がするけど相記述に利用できるの？\\
    A.その後にどっちかとるProjection作用させればいい。計算量は所詮O(N)くらいになるので無視していい。なんなら最適化とかに応用する場合はエネルギーだけが重要なのでProjectionの必要すらない。
    \item Q.どういう場合により優位になる？（Future work）ナイーブには有限でギャップが０の場合は否応なく混じる気がしてて優位になる気がするが、実はそのような系は非常に限られることが知られている。詳しくは後述。
\end{enumerate}
...
\section{鈴木・トロッター分解}
量子系の時間発展を量子回路上で実装する際、Hamiltonian 全体の指数演算子を直接実装することは一般には容易ではない。一方、Hamiltonian が
\begin{align}
    H=\sum_{j=1}^{n}H_j
\end{align}
のように、それぞれの時間発展演算子 $e^{-iH_jt}$ を比較的容易に実装できる項へ分解されている場合には、各項による時間発展を逐次的に作用させることによって、全体の時間発展を近似することができる。この方法が鈴木・トロッター分解である。

まず、その基本となる二つの演算子に対する関係を示す。

\begin{theorembox}{Baker--Campbell--Hausdorff 展開}
二つの演算子 $A,B$ と十分小さい実数 $t$ に対して、
\begin{align}
    e^{tA}e^{tB}
    =
    \exp\left(
...
\begin{align}
    ...
    (A+B)^2+[A,B]
\end{align}
であるため、$f(t)$ を $t=0$ のまわりで Taylor 展開すると、
\begin{align}
    f(t)
    =
    I
...
以上の関係から、一次の鈴木・トロッター分解を導くことができる。

\begin{theorembox}{一次鈴木・トロッター分解}
二つの演算子 $A,B$ に対して、
\begin{align}
    e^{A+B}
    =
...
\begin{proofbox}
Baker--Campbell--Hausdorff 展開において、
\begin{align}
    t=\frac{1}{r}
\end{align}
とすると、
...
この関係は、有限個の演算子に対しても拡張することができる。

\begin{theorembox}{一次鈴木・トロッター分解の一般化}
有限個の演算子 $A_j$ に対して、
\begin{align}
    \exp\left(
        \sum_{j=1}^{n}A_j
...
次に、この公式を量子系の時間発展に適用する。Hamiltonian を
\begin{align}
    H=\sum_{j=1}^{n}H_j
\end{align}
とする。

このとき、時間 $t$ における時間発展演算子は
\begin{align}
    U(t)
    =
    e^{-iHt}
...
\subsection{一次鈴木・トロッター分解の誤差}

有限の分割数 $r$ を用いる場合には、鈴木・トロッター分解に由来する誤差が生じる。まず、簡単のため
\begin{align}
    H=H_1+H_2
\end{align}
の場合を考える。

Baker--Campbell--Hausdorff 展開に
\begin{align}
    A=-iH_1,
    \qquad
    B=-iH_2
...
しかし、全時間 $t$ の時間発展を構成するためには、この近似を $r$ 回繰り返す必要がある。そのため、全体の誤差は単純に $O(r^{-2})$ とはならず、各時間ステップにおける誤差の蓄積を考慮する必要がある。
...
\begin{align}
    ...
    U_{\Delta t}^{r-1-\ell}
    \left(
        U_{\Delta t}-V_{\Delta t}
    \right)
    V_{\Delta t}^{\ell}
\end{align}
という恒等式が成り立つ。

両辺の演算子ノルムを取ると、三角不等式および演算子ノルムの劣乗法性より、
\begin{align}
    \left\|
        U_{\Delta t}^{r}
        -
...
$U_{\Delta t}$ および $V_{\Delta t}$ はともにユニタリ演算子であるため、
\begin{align}
    \left\|
        U_{\Delta t}^{m}
    \right\|
...
\begin{align}
    ...
    \right)
\end{align}
であるが、それを $r$ 回繰り返すため、全時間発展に対する誤差は $O(r^{-1})$ となる。

より一般に、
\begin{align}
    H=\sum_{j=1}^{n}H_j
\end{align}
の場合、一次鈴木・トロッター分解における主要な誤差は Hamiltonian の各項の非可換性によって決定される。具体的には、
\begin{align}
    \epsilon_{\mathrm{Trotter}}
    =
    O\left(
...
特に、すべての Hamiltonian 項が互いに可換、すなわち
\begin{align}
    [H_j,H_k]=0
    \qquad
    \text{for all } j,k
...
\begin{align}
    ...
    e^{-iH_jt}
\end{align}
が厳密に成立するため、鈴木・トロッター分解による誤差は生じない。

\begin{resultbox}{一次鈴木・トロッター分解の誤差と分割数}

一次鈴木・トロッター分解では、一ステップあたりの誤差は
\begin{align}
    \epsilon_{\mathrm{step}}
    =
    O\left(
    ...
    \right)
\end{align}
である。一方、全時間発展を構成するためにはこの操作を $r$ 回繰り返す必要があるため、全体の誤差は
\begin{align}
    \boxed{
        \epsilon_{\mathrm{Trotter}}
        =
...
したがって、時間発展を誤差 $\epsilon$ 以下で近似するために必要な分割数は、
\begin{align}
    \boxed{
        r
        =
...
また、一つの Trotter step を実装するために必要なゲート数を $G_{\mathrm{step}}$ とすると、全時間発展を実装するために必要なゲート数は
\begin{align}
    G
    =
    O\left(
    ...
    \right)
\end{align}
である。したがって、一次鈴木・トロッター分解によるゲート計算量は
\begin{align}
    G
    =
    O\left(
    ...
    \right)
\end{align}
となる。ただし、Hamiltonian の非可換性を明示する場合には、
\begin{align}
    G
    =
    O\left(
