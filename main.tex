...
input:(A,B,C,D)及びそのL次べき展開
\begin{align*}
    A(\lambda)&=\sum_{n=0}^{L}a_{n}x^{n}\\
    B(\lambda)&=\sum_{n=0}^{L}b_{n}x^{n}\\
    C(\lambda)&=\sum_{n=0}^{L}c_{n}x^{n}\\
...
\begin{enumerate}
    \item $(A,B,C,D)$から$\Phi_{L,j}$を逆引き
        ...
        \Phi_{L,j}=
        \begin{cases}
            i^{j}\sum_{n=0}^{L-j}(a_{n}+ib_{n}){}_{\frac{L-n}{2}}C_{\frac{j}{2}}\quad (j\in odd)\\
            i^{j}\sum_{n=0}^{L-j}(ic_{n}-id_{n}){}_{\frac{L-n}{2}}C_{\frac{j-n}{2}}\quad (j\in even)
        \end{cases}
        \end{align*}
        ただし、${}_{i}C_{j>i}=0$とする。
    \item $\phi_{L}$及び$\Phi_{L-1,j}$を逆引き
    \begin{align*}
