...
\end{align*}
また、時間発展回路自体もTrotter近似を用いているため、その分割分の誤差が存在する。この誤差を$\epsilon_{tro}$程度に抑えるために分割数を$O(Tlog\frac{1}{\epsilon_{tro}})$とする。時間発展1step分の回路の計算量はO(N)であるため、
\begin{resultbox}{断熱オンリーの場合の必要計算量}
\begin{align*}
    \text{(断熱準備の計算量)}=O(N)\times O(Tlog\frac{1}{\epsilon_{tro}})=O(N^{3}log\frac{1}{\epsilon}log\frac{1}{\epsilon_{tro}})
\end{align*}
\end{resultbox}
...
\begin{align*}
    \text{(初期状態用意クエリ計算量)}=O(N^{1+a}log\frac{1}{\epsilon_{tro}})
\end{align*}
...
\begin{resultbox}{$T=O(N^{a})$のハイブリットアルゴリズムの計算量}
    \begin{align*}
    \text{(hybridの計算量)}=&O(N^{1+a}exp(N^{1-\frac{a}{2}})log\frac{1}{\epsilon_{tro}}) \\
...
\begin{resultbox}{ハイブリットアルゴリズムの最良計算量}
    \begin{align*}
        O(N^{3}log\frac{1}{\epsilon_{tro}})+O(Nlog\frac{1}{\epsilon})
    \end{align*}
\end{resultbox}
